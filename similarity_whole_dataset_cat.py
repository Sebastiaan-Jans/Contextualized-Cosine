...
representatoins. This code trains on the whole dataset, as opposed per category,
as is the case in similarity_per_cat.py
...
class Symmetric_BilinearNN(torch.nn.Module):
    ...
    # compute the input values on the fully-connected layer
    def forward(self, inputs1, inputs2):
        return (torch.mm(self.fc1(inputs1), torch.transpose(self.fc1(inputs2), 0, 1))) / (torch.sqrt(
            ((torch.mm(self.fc1(inputs1), torch.transpose(self.fc1(inputs1), 0, 1)))) * torch.sqrt(
                torch.mm(self.fc1(inputs2), torch.transpose(self.fc1(inputs2), 0, 1)))))
...
def plot_losses(cat_name, model_name, train_losses, val_losses, i, path):
    ...
    plt.legend()
    plt.savefig(path + "/imgs/" + cat_name + " Fold number {}".format(i + 1))
    plt.close()
    return plt
...
def compare(category, n1, n2, ctxt=False):
    ...
    for i in range(len(human_judgements[category])):
        if (human_judgements[category][i][0][0] == n1 and human_judgements[category][i][0][1] == n2) or (
                human_judgements[category][i][0][0] == n2 and human_judgements[category][i][0][1] == n1):
...
def passMaskedSentence(sent, modelLM, tokenizer):
    ...
    input_ids = tokenizer.convert_tokens_to_ids(
        ['[CLS]'] + tokenizer.tokenize(sent.replace('mask', '[MASK]')) + ['[SEP]'])
...
def get_model(model_name, context_on):
    ...
    if model_name == "GLOVE":
        ...
        raise CustomError(
            "Trying to use a model that is not known. Please choose between GLOVE, word2vec, GPT-2 or BERT.")
...
def gpt2_model_ctxt(gpt2Model, tokenizer, gpt2_category="Birds"):
    if gpt2_category not in ['Birds', 'Clothing', 'Fruit', 'Furniture', 'Professions', 'Sports', 'Vegetables',
                             'Vehicles']:
        raise CustomError(
            "You did not use one of the eight possible categories for the gpt2 category. Please use one of the following: 'Birds', 'Clothing', 'Fruit', 'Furniture', 'Professions', 'Sports', 'Vegetables', 'Vehicles'.")
...
def make_save_dir(model_name, context_on):
    ...
    if context_on and (model_name == "GPT-2" or model_name == "BERT"):
        ...
        ctxt_str = ""
    path = os.path.join(os.getcwd(), "Data_whole_dataset_" + model_name + ctxt_str)
...
# ## Train k models on the complete dataset
def train_big(categories, n_epochs):
    # Create the data set
    data = defaultdict()
    for cat in categories:
        data[cat] = create_dataset(cat)
    dataset = data["Birds"][0]
    ...
    # Run everything for each fold
    for i in range(len(data['Birds'][1])):
        print("Fold " + str(i+1) + "\n \n")

        # Set lists per fold to empty
        tot_train_fold = []
        tot_val_fold = []
        model_fold = []

        # Model_bool ensures that if an error occurs, the fold starts again
        model_bool = True
        while model_bool is True:
            ...
            # Set lists per epoch to empty
            tot_train_epoch = []
            tot_val_epoch = []

            # Set variables to prevent overfitting to 0
            lowest_epoch_val_loss = 0
            increasing_loss_counter = 0

            # Loop for number of specified epochs
            for n in range(n_epochs):

                # Print statement every 100th epoch
                if n % 100 == 0:
                    print("Number of epochs: " + str(n+1))
                    if n != 0:
                        print("Loss = ", np.mean(tot_train_epoch))

                # Train the model once on all the data in each category
                # Every epoch this is in the same order to prevent overfitting
                for cat in categories:
                    model_bi, total_train_losses, model_bool, total_val_losses, epoch_cutoff, lowest_loss_fold = run_epochs(model_bi, criterion, optimizer, 1, data[cat][1][i], data[cat][2][i])

                    # If errors occur break
                    if model_bool:
                        break
                    # Otherwise add losses for this category to the epoch losses
                    tot_train_epoch.append(total_train_losses)
                    tot_val_epoch.append(total_val_losses)

                # If errors occurred, break once more
                if model_bool:
                    break

                # To prevent overfitting, if the model does not improve for five consecutive epochs
                # it will stop training.
                if np.mean(tot_train_epoch) < 0.1 and lowest_epoch_val_loss <= np.mean(tot_val_epoch):
                    # loss is increasing or stays the same
                    increasing_loss_counter += 1
                elif np.mean(tot_val_epoch) < 0.1 and lowest_epoch_val_loss > np.mean(tot_val_epoch):
                    # loss is decreasing
                    increasing_loss_counter = 0
                    lowest_epoch_val_loss = np.mean(tot_val_epoch)
                else:
                    lowest_epoch_val_loss = np.mean(tot_val_epoch)

                if increasing_loss_counter == 10:
                    print("Epoch stopped before max number. Current epoch = "+str(n-9))

                    # Remove models and loss values that were no longer relevant
                    # as the quality of the model did not improve
                    model_fold = model_fold[:-8]
                    tot_train_fold = tot_train_fold[:-8]
                    tot_val_fold = tot_val_fold[:-8]
                    used_epochs.append(n-8)
                    break

                # Save the new model to the list of models
                model_fold.append(copy.deepcopy(model_bi))

                # Average the losses for all the categories and add to fold list
                tot_train_fold.append(np.mean(tot_train_epoch))
                tot_val_fold.append(np.mean(tot_val_epoch))

                # If no earlier cutoff is reached, the max number of epochs is added to list
                if n == n_epochs-1:
                    used_epochs.append(n_epochs)

        # Add fold values to the total lists
        models.append(model_fold)
        tot_train.append(tot_train_fold)
        tot_val.append(tot_val_fold)
...
def get_correlations_big(models, data, categories, used_epochs, num_folds):
    ...
    for i in range(num_folds):
        # Get the most recent model and the number of epochs the model was trained
        used_epoch = used_epochs[i]
        model = models[i][-1]
...
def get_baseline_big(data, categories, num_folds):
    ...
    for i in range(num_folds):
        ...
        for cat in categories:
            ...
            for X1_batch, X2_batch, y_batch in data[cat][2][i]:
                y_pred = torch.mm(X1_batch, torch.transpose(X2_batch, 0, 1)) / \
                         (torch.sqrt((torch.mm(X1_batch, torch.transpose(X1_batch, 0, 1)))) *
                          torch.sqrt(torch.mm(X2_batch, torch.transpose(X2_batch, 0, 1))))
...
# copy of the create_dataset function, used for making the wordsim and simlex test sets
def create_dataset_extra(cat):
    ## Bilinear
...
for model_name, context_on in all_models:
    for num_folds in [5, 6, 7]:
        for learning_rate in [0.00001, 0.000001, 0.0000001]:
            ...
            # train the models
            tot_train, tot_val, models, used_epochs, datas = train_big(categories, num_epochs)
    ...
    # A for-loop to find the best hyperparameter combination per model and category
    # Dictionaries are used to save all the different model/cat/lr/fold combinations, this for-loop loops through them
    # to find the lr+fold combination with the highest correlations for every model and category combination
    for key in corr_p_dict.keys():
        model, cat = key.split()
        highest_corr_p = 0
        highest_corr_s = 0
        ...
        best = [model] + [cat] + [lr_corr_p] + [folds_corr_p] + [lr_corr_s] + [folds_corr_s]
        best_params.append(best)
    print(best_params)

    # save the best hyperparameter combinations in a csv file
    with open("hyperparameter_results_wholedataset.csv", "w", newline='') as f:
        writer = csv.writer(f)
